#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""D6-v3 (artifact-complete rerun of v2): full-profile per-ray decoder with FiLM conditioning.
...
def main():
    ...
    # end-to-end: save full profiles + argmax/soft-argmax depth + checkpoint
    os.makedirs(os.path.join(ROOT, "d6_pred_v3"), exist_ok=True)
    torch.save(model.state_dict(), os.path.join(ROOT, "d6_pred_v3", "model.pt"))
    model.eval()
    with torch.no_grad():
        for n in val_names[:12]:
            ...
            np.save(os.path.join(ROOT, "d6_pred_v3", n + ".npy"),
                    pr.numpy().astype(np.float32))       # hard-argmax bin depth
            np.save(os.path.join(ROOT, "d6_pred_v3", n + "_soft.npy"),
                    soft[0].numpy().astype(np.float32))  # soft-argmax bin depth
            np.save(os.path.join(ROOT, "d6_pred_v3", n + "_prof.npy"),
                    pred[0].numpy().astype(np.float32))  # full 96-bin profile
            np.save(os.path.join(ROOT, "d6_pred_v3", n + "_cov.npy"), cov.numpy())
    print("done. profiles+depths+checkpoint -> d6_pred_v3/", flush=True)
