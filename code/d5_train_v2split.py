...
def main():
    ...
    os.makedirs(os.path.join(ROOT, "d5_pred_v2split"), exist_ok=True)
    model.eval()
    with torch.no_grad():
        for n in val_names[:8]:
            imgs, feats, peak, cov, rmax = va[n]
            pred = model(imgs[None].to(device), feats[None].to(device))[0].cpu().numpy()
            np.save(os.path.join(ROOT, "d5_pred_v2split", n + ".npy"), pred)
    print("done. predictions -> d5_pred_v2split/", flush=True)
