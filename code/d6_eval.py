#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Corrected eval for D6 predicted depth fields (bin-index format).
Usage: python d6_eval.py <pred_dir> [cov_floor]
Pred files: per-object .npy holding per-ray depth as a float in [0, N_BINS-1]
(bin index of soft/hard argmax). Converts to normalized depth = bin/(N_BINS-1),
then radius = depth*rmax, builds points, and scores Chamfer/F@t vs GT
surf_points (D5 assets). Also reports depth-med normalized vs depth_peak/rmax.
"""
import json
import os
import sys
...
def main():
    ...
    for f in files:
        name = f[:-4]
        pred = np.load(os.path.join(pred_dir, f)).astype(np.float32)  # bin index [0,95]
        ...
        r = (pred / (N_BINS - 1)) * rmax  # normalized depth -> absolute radius
        keep = (cov >= cov_floor) & (r >= 0.02) & (r < rmax)
        pc = (r[keep, None] * dirs[keep]).astype(np.float32)
        dep = np.abs(pred / (N_BINS - 1) - peak / rmax)
