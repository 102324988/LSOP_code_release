...
SHARPENING-TARGET VARIANT (M4): train the same per-ray head against a
p^gamma-sharpened GT profile (sh ** target_gamma, still max=1). Post-hoc
p^gamma (m3b_pgamma) showed the M3b width gap is an OBJECTIVE artifact, not
an information limit: gamma=2 halves FWHM to GT level AND improves dmed_s
(soft->hard recovery) at the cost of raw peak. A learned sharp target should
produce natively narrow, high-peak profiles (the M4-design "sharpening
target" candidate), while keeping depth.
...
every iteration: ~8 min/epoch -> 40 epochs unaffordable):
  ...
  --stage train  light head (ray_proj + per-ray FiLM decoder), D8-decoder
                 init, pure v3 L1 on the sharpened target, features loaded.
...
Usage:
  python m3b_train_sg.py --stage train --out m3b_preray_sg2 --target_gamma 2 \
      [--epochs 30]
"""
import argparse
import json
...
def main():
    ...
    ap.add_argument("--target_gamma", type=float, default=1.0,
                    help="sharpen GT target by sh**gamma before L1 (1.0 = plain)")
    ap.add_argument("--rdim", type=int, default=128,
                    help="per-ray FiLM channel dim (sensitivity: 128 vs 512)")
    ...
    if args.stage in ("train", "all"):
        ...
        m2 = Model2(rdim=args.rdim).to(device)
        ...
        for ep in range(args.epochs):
            ...
            for b0 in range(0, len(names_list), args.batch):
                ...
                sh_t = sh ** args.target_gamma
                loss = (w * (pred - sh_t).abs()).mean()
        ...
        with open(os.path.join(ROOT, args.out, "meta.json"), "w") as f:
            json.dump({"epochs": args.epochs, "batch": args.batch, "lr": args.lr,
                       "r_ref": args.r_ref, "rdim": args.rdim, "loss": "v3-L1",
