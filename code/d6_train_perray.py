#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""D6: full-profile per-ray decoder. Multi-view encoder -> global feature;
per-ray MLP (global feat + ray positional encoding) -> 96-bin spherical
profile. Supervised on normalized profile shape (per-ray max-norm) with
coverage weighting + depth_peak consistency (Huber). Eval: val profile L1,
depth med-abs, and end-to-end surface recovery (argmax -> points).
Usage: python d6_train_perray.py [--n_train N] [--n_val N] [--epochs E]
                                 [--views K] [--lr L] [--seed S]
"""
import argparse
import json
...
def ray_encodings():
    ...
    # positional encoding: 3 freqs on each component
    pe = []
...
def load_object(name, n_views, rng):
    ...
    # normalize profile shape per ray (peak = 1)
...
class Model(nn.Module):
    def __init__(self, gdim=512, nb=N_BINS):
        ...
        self.decoder = nn.Sequential(
            nn.Linear(gdim + PE_DIM, 512), nn.ReLU(),
    ...
    def forward(self, imgs, feats, ray_pe, chunk=2048):
        ...
        for r0 in range(0, R, chunk):
            pe = ray_pe[r0:r0 + chunk].to(imgs.device)  # (C,21)
            C = pe.shape[0]
            inp = torch.cat([g[:, None, :].expand(-1, C, -1),
                             pe[None, :, :].expand(B, -1, -1)], dim=-1)  # (B,C,533)
            outs.append(self.decoder(inp))
...
def main():
    ...
    # baseline: global mean profile shape
    gs = torch.stack([tr[n][2] for n in train_names]).mean(0)  # (R,96)
    base_med = []
    for n in val_names:
        sh, cov, peak, _ = va[n][2], va[n][3], va[n][4], va[n][5]
        m = cov >= 0.02
        e = (gs[m] - sh[m]).abs()
        base_med.append(float(e.mean()))
    print("BASELINE global-mean profile L1=%.4f" % np.median(base_med), flush=True)
    ...
    for ep in range(args.epochs):
        ...
        for b0 in range(0, len(train_names), args.batch):
            ...
            w = (cov + 0.05).clamp(max=1.0).unsqueeze(-1)  # coverage weighting
            lp = (w * (pred - sh).abs()).mean()
            # depth consistency: argmax of predicted profile vs GT peak
            pr = pred.detach()
            pred_peak = ((pr * torch.arange(N_BINS, device=device)[None, None, :]).sum(-1)
                         / (pr.sum(-1) + 1e-6))  # soft argmax
            ld = lossf(pred_peak * w.squeeze(-1), peak * w.squeeze(-1))
            loss = lp + 0.5 * ld
        ...
        with torch.no_grad():
            for n in val_names:
                imgs, feats, sh, cov, peak, _ = va[n]
                pred = torch.sigmoid(model(imgs[None].to(device),
                                           feats[None].to(device), ray_pe))[0].cpu()
                m = cov >= 0.02
                pl1.append(float((pred[m] - sh[m]).abs().mean()))
                pr = (pred * torch.arange(N_BINS)[None, :]).sum(-1) / (pred.sum(-1) + 1e-6)
                dmed.append(float((pr[m] - peak[m]).abs().median()))
    ...
    os.makedirs(os.path.join(ROOT, "d6_pred"), exist_ok=True)
    model.eval()
    with torch.no_grad():
        for n in val_names[:12]:
            ...
            pred = torch.sigmoid(model(imgs[None].to(device),
                                       feats[None].to(device), ray_pe))[0].cpu()
            pr = (pred * torch.arange(N_BINS)[None, :]).sum(-1) / (pred.sum(-1) + 1e-6)
            np.save(os.path.join(ROOT, "d6_pred", n + ".npy"),
                    pr.numpy().astype(np.float32))  # normalized depth field (R,)
            np.save(os.path.join(ROOT, "d6_pred", n + "_cov.npy"), cov.numpy())
    print("done. predictions -> d6_pred/", flush=True)
